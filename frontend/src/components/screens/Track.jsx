...
function Track() {
    ...
    if (urlId === "") {
      ...
      if (mainExtractAns === "") {
        ...
        let result = await fetch(import.meta.env.VITE_HANDEL_HISTORY, {
          ...
          body: JSON.stringify({ id: mainExtractAns })
        })
        setProgress(50)
        // checking result
        if (result) {
  ...
  return (
    <>
      <div className="h-lvh w-full bg-gradient-to-br from-[rgb(12,31,39)] to-[rgb(10,15,24)] font-sans pt-3">
        <LoadingBar
        ...
        {status && <div className="flex w-3/4 m-auto my-5 flex-col py-2 h-4/5  overflow-hidden overflow-y-scroll">
          <h2 className='text-white text-xl font-semibold px-4 my-4'>Total Clicks : {totalClicks}</h2>

          <table className='table-auto border-separate border-2 border-slate-500'>
            <caption class="caption-top mb-3 font-semibold text-white">
              Click History
            </caption>
            <tr>
