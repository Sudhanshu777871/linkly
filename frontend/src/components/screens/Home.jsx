        ...
        if (urlId === "") {
            ...
            if (urlPattern.test(urlId)) {
                ...
                let result = await fetch(import.meta.env.VITE_NEW_URL, {
                    ...
                    body: JSON.stringify({ originalURL: urlId })
                })
                // checking result
                if (result) {
                    setProgress(50)
        ...
        <>
            <div className={`h-lvh w-full ${isDark ? `bg-white` : `bg-gradient-to-br from-[rgb(12,31,39)] to-[rgb(10,15,24)]`}  font-sans flex flex-col lg:justify-between md:justify-between`}>
                ...
                <nav className='flex justify-between items-center py-3'>
                    ...
                    <button className={`px-8 py-1.5 ${isDark ? `bg-white` : `bg-[rgb(24,30,41)]`} ${isDark ? `text-[rgb(24,30,41)]` : `text-white`}   shadow-sm shadow-gray-400 text-2xl mx-5 rounded-full border-2 border-gray-400 hover:ring`} onClick={mode}> {isDark ? <p><i className
                        ={`fa fa-moon-o ${isDark ? `text-[rgb(24,30,41)]` : `text-white`} text-2xl mx-1`}></i>Dark</p> : <p><i className
                            ={`fa fa-sun-o ${isDark ? `text-[rgb(24,30,41)]` : `text-white`} text-2xl mx-1`}></i>Light</p>}</button>
                </nav>
                ...
                <div>
                    ...
                    <div className='flex justify-center'>
                        ...
                        {urlStatus && <button className={`${isDark ? `bg-white border-2 border-[rgb(50,62,89)] text-[rgb(50,62,89)]` : `bg-[rgb(50,62,89)] border-2 border-[rgb(50,62,89)] text-white`} mx-2 mt-3 text-white px-4 py-2 text-xl rounded-sm hover:ring`} onClick={downloadQRCode}> Generate QR </button>}
                    </div>
